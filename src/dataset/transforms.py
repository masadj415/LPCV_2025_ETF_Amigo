...
# Augmentisuca transformacija koja se koristi pri treningu
...
RANDAUG       = transforms.Compose([
    ...
    transforms.Resize((224, 224)),  # Ensures output is 224x224
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
])
...
# Transformacija koja samo prebacuje PIL sliku u tenzor
...
# Transformacija koja se koristi pri inferencu (treba da se zapravo posle doda i u model, jer oni ne salju normalizovane slika na AIHUB
# Normalizuje i resizuje sliku
# Doduse oni ce slati nenormalizovan tenzor, a ne PIL
